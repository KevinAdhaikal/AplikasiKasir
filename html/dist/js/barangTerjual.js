        ...
        if (response.status == 200) {
            await response.text().then(data => {
                ...
                for (let a = 0; a < data.length - 1; a++) {
                    ...
                    $("#barangTerjualTable").DataTable().row.add([valueSplit[0], valueSplit[1], Intl.NumberFormat('id', {}).format(Number(valueSplit[2].replaceAll(".", ""))), "Rp" + Intl.NumberFormat('id', {}).format(Number(valueSplit[3].replaceAll(".", ""))), "Rp" + Intl.NumberFormat('id', {}).format(Number(valueSplit[4].replaceAll(".", ""))), `
                    <center>
                    <button type="button" class="btn btn-danger" onclick="hapusBarangTerjual(${valueSplit[0]}, '${dateValue ? dateValue.replaceAll("-", "_") : ""}')">Hapus</button>
                    </center>`]).draw(false)
                    tempPembukuan[0] += Number(valueSplit[2].replaceAll(".", ""))
                    tempPembukuan[1] += Number(valueSplit[3].replaceAll(".", ""))
                    tempPembukuan[2] += Number(valueSplit[4].replaceAll(".", ""))
                }
