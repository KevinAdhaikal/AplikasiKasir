        ...
        if (response.status == 200) {
            response.text().then(data => {
                ...
                for (let a = 0; a < data.length - 1; a++) {
                    ...
                    $("#pengeluaranTable").DataTable().row.add([valueSplit[0], valueSplit[1], "Rp" + Intl.NumberFormat('id', {}).format(Number(valueSplit[2].replaceAll(".", ""))), `
                    <center>
                    <button type="button" class="btn btn-danger" onclick="hapusPengeluaran(${valueSplit[0]}, '${dateValue ? dateValue.replaceAll("-", "_") : ""}')">Hapus</button>
                    </center>`]).draw(false)
                    tempPengeluaran += Number(Number(valueSplit[2].replaceAll(".", "")))
                }
