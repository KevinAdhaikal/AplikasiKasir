    ...
    switch(sb_convert_var_to_int(e->stream, "barangMasukArgs")) {
        case 1: {
            ...
            if (tanggalBarangMasuk[0]) {
                ...
                sprintf(tempString, "SELECT rowid,waktu,nama,jumlah,harga from barangMasuk_%s", tanggalBarangMasuk);
            }
            else sprintf(tempString, "SELECT rowid,waktu,nama,jumlah,harga from barangMasuk_%d_%d_%d", timeinfo->tm_mday, timeinfo->tm_mon + 1, timeinfo->tm_year + 1900);
        ...
        case 3: {
            ...
            sqlite3_bind_int64(statement, 3, atoll(hargaBarang));
            ...
            sqlite3_bind_int64(statement, 1, atoll(jumlahBarang));
            sqlite3_bind_text(statement, 2, namaBarang, -1, SQLITE_STATIC);
        ...
        case 4: {
            char namaBarang[255];
            char barangMasukID[32];

            sb_get_header(e->stream, "namaBarang", namaBarang, 254);
            sb_get_header(e->stream, "tanggalBarangMasuk", tanggalBarangMasuk, 11);
            sb_get_header(e->stream, "barangMasukID", barangMasukID, 31);
            ...
            if (tanggalBarangMasuk[0]) {
                ...
                sprintf(tempString, "SELECT jumlah FROM barangMasuk_%s where rowid = ?", tanggalBarangMasuk);
            }
            else sprintf(tempString, "SELECT jumlah FROM barangMasuk_%d_%d_%d WHERE rowid = ?", timeinfo->tm_mday, timeinfo->tm_mon + 1, timeinfo->tm_year + 1900);
            ...
            sqlite3_prepare_v2(db, tempString, -1, &statement, NULL);
            sqlite3_bind_int64(statement, 1, atoll(barangMasukID));
            sqlite3_step(statement);
            sqlite3_finalize(statement);
            sqlite3_close(db);
            ...
            sqlite3_bind_int64(statement, 1, atoll(rowBack.rows));
            sqlite3_bind_text(statement, 2, namaBarang, -1, SQLITE_STATIC);
            sqlite3_step(statement);
            sqlite3_finalize(statement);
